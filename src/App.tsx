...
function ResultsScreen({inputs,cityName,onBack,onProfile,onOpenDetail,targetBand,weatherInfo,t}:any){
  ...
  if(!loading&&!error&&cards.length===0) return(<div style={{position:"relative",zIndex:1,padding:"52px 24px",textAlign:"center"}}><p style={{fontSize:48,margin:"0 0 16px"}}>🦴</p><h2 style={{fontSize:20,fontWeight:800,color:t.text,margin:"0 0 8px",letterSpacing:-0.5}}>Vybe came up empty</h2><p style={{fontSize:14,color:t.textMuted,margin:"0 0 28px",lineHeight:1.5}}>Nothing matched your exact filters right now. But don't let that stop your day.</p><button onClick={onBack} style={{width:"100%",backgroundColor:t.text,color:t.bg,border:"none",borderRadius:14,padding:"15px 24px",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginBottom:12}}>Loosen the filters →</button><button onClick={()=>load()} style={{width:"100%",backgroundColor:t.surface,color:t.textMuted,border:`1.5px solid ${t.border}`,borderRadius:14,padding:"14px 24px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Try again</button></div>);
